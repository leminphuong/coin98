        ...
        <div className="transition-all duration-300 bg-background lg:hover:bg-background-hovered article-horizontal select-none overflow-hidden p-300">
          ...
          <div className="h-400 flex items-center justify-between mt-200">
            ...
            <button className="p-100 rounded-050 bg-button-ghost-background">
              <i className="ab-icon text-size-800 ab-bookmark_outlined" />
            </button>
          </div>
        </div>
      </Link>
...
export default function DetailLearnPage() {
  ...
  useEffect(() => {
    ...
    fetch(
      `/wp-json/toan/v1/learn-search?${params.toString()}`
    )
      .then((res) => res.json())
      ...
      <div className="fixed bg-background z-3 w-full">
        <div className="py-200 flex max-w-w1440 mx-auto">
          ...
          <div className="flex-1 px-300">
            <label className="h-600 bg-background flex items-center px-200 rounded-full border-0125 border-border focus-within:border-cbr-50">
              <i className="ab-icon mr-150 text-size-800 ab-search" />
              <input
          ...
          <div className="max-w-w728 w-full">
            ...
            <div className="flex flex-col">
              {!loading && totalPosts === 0 && (
                <div className="px-300 py-600 text-center text-text-subtlest">
                  No results found
                </div>
              )}
