...
/* ============================================
   DESKTOP CARD (Hover overlay giống 100% Upside)
=============================================== */
export function SeriesCardDesktop({ item }: { item: SeriesItem }) {
  return (
    <a href={item.url} className="block group/seriesCard">
      <div className="transition-all duration-300 bg-background hover:bg-background-hovered p-300 rounded-075">

        <div className="relative overflow-hidden rounded-050 w-full aspect-square">

          {/* IMAGE */}
          <img
            src={item.image}
            alt={item.title}
            className="w-full h-full object-cover rounded-050 transition-all duration-200 ease-linear group-hover/seriesCard:scale-105"
          />

          {/* OVERLAY */}
          <div className="
            absolute inset-x-0 bottom-0 rounded-050 bg-series-overlay
            px-300 pb-300 pt-200
            flex flex-col justify-end
            top-[40%]
            transition-all duration-200 ease-linear
            group-hover/seriesCard:top-0
          ">
            
            {/* TITLE GROUP (mờ dần khi hover) */}
            <div
              data-theme="dark"
              className="
                select-none flex-1 relative flex flex-col justify-end
                transition-all duration-200
                delay-200
                group-hover/seriesCard:opacity-0
                group-hover/seriesCard:delay-0
              "
            >
              <div className="text-text-primary article-h4">{item.title}</div>
              <div className="ui-text-x-small text-text-secondary mt-100">
                {item.posts} POSTS
              </div>
            </div>

            {/* DESCRIPTION (xuất hiện khi hover) */}
            <div
              data-theme="dark"
              className="
                ...
                transition-all duration-200 ease-linear
                delay-0
                group-hover/seriesCard:translate-y-0
                group-hover/seriesCard:opacity-100
                group-hover/seriesCard:delay-100
              "
            >
              {item.description}
            </div>
            ...
            >
              ...
              <div className="overflow-hidden relative w-max h-max group/tooltip hover:overflow-visible">
                <button className="
                  group/ab-button relative flex items-center justify-center select-none
                  rounded-050 p-100 border-0125 border-transparent
                  bg-button-ghost-background hover:bg-button-ghost-background-hovered
                  transition-all duration-200
                ">
                ...
                <div className="
                  absolute z-10 w-max px-075 py-050 text-size-400 rounded-050
                  border border-tooltip-background bg-tooltip-background text-tooltip-text
                  shadow-elevation-none mb-150 bottom-100% left-1/2 -translate-x-1/2
                  transition-transform
                  translate-y-full group-hover/tooltip:translate-y-0
                ">
                  Copy link
                </div>
              </div>
              ...
              <button className="
                group/ab-button relative flex items-center justify-center select-none
                rounded-050 border border-button-tertiary-border
                py-050 px-100
